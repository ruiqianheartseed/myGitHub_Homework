...
static void forward(LeNet5 *lenet, Feature *features, double(*action)(double))
{
	...
	DOT_PRODUCT_FORWARD(features->layer5, features->layer6, lenet->weight5_6, lenet->bias5_6, action);
	DOT_PRODUCT_FORWARD(features->layer6, features->output, lenet->weight6_7, lenet->bias6_7, action);
}
//定义全网络的反向参数
static void backward(LeNet5 *lenet, LeNet5 *deltas, Feature *errors, Feature *features, double(*actiongrad)(double))
{
	DOT_PRODUCT_BACKWARD(features->layer6, errors->layer6, errors->output, lenet->weight6_7, deltas->weight6_7, deltas->bias6_7, actiongrad);
	DOT_PRODUCT_BACKWARD(features->layer5, errors->layer5, errors->layer6, lenet->weight5_6, deltas->weight5_6, deltas->bias5_6, actiongrad);
...
void Initial(LeNet5 *lenet)
{
	...
	for (double *pos = (double *)lenet->weight5_6; pos < (double *)lenet->bias6_7; *pos++ *= sqrt(6.0 / (LAYER5 + LAYER6)));
	for (double *pos = (double *)lenet->weight6_7; pos < (double *)lenet->bias0_1; *pos++ *= sqrt(6.0 / (LAYER6 + OUTPUT)));
